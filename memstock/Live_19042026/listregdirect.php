...
$vsql="select * from m_anggota where 1 and fidregistrar ='$vUser' ";
...
*/
	...
	<div class="right_col" role="main">
		...
		<div><label><h3>Daftar Jamaah yang Didaftarkan oleh <? if ($vLevel=='SUBKOR') echo 'Sub'; ?> Korwil <?=$vUser?></h3></label></div>
...
<ul class="pagination" >
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($idisp!=$vPage) {
?>
              <li ><a  href="listregdirect.php?lmAktif=<?=$vAktif?>&lmMship=<?=$vPrem?>&uPage=<?=$idisp?>&lmSort=<?=$vSort?>&current=<?=$_REQUEST['current']?>&uRegistrar=<?=$vUser?>" >
              <?=$i+1?>
              </a> </li> 
              <?
  } else {
