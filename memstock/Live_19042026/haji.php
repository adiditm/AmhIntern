...
<style type="text/css">
  ...
  //    $vAnd.=" and fpaket=$vPack ";
   ...
   if ($vHari !='')
    ...
    $vSQL="select * from m_tour where fstatusrow=1 and ftgldepart >= date(now()) and date(fexpired) >= date(now()) and fgroup='h'  $vAnd";
   $db->query($vSQL);
...
<div class="row">
 ...
 <div class="col-lg-6"><label>Jumlah Hari </label>
   ...
   <? for ($i=1;$i<41;$i++) {?>
<option value="<?=$i?>" <? if ($vHari==$i) echo 'selected';?>><?=$i?></option>
<? } ?>
</select>
   </div>
   <div>&nbsp;</div>
...
<?
  $vSQL="select * from m_tour where fstatusrow=1 and  ftgldepart >= date(now()) and date(fexpired) >= date(now()) and fgroup='h'  $vAnd order by fcountry";
