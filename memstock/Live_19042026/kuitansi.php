...
function Terbilang ($angka) {
		...
		if (is_numeric($vAngs))
				 $vPayFor="Biaya Umroh angsuran ke $vAngs";
		  else if ($vAngs == 'lns') 
				  $vPayFor="Biaya Pelunasan Umroh";
		  else if ($vAngs == 'str') 
				  $vPayFor="Biaya Setoran Awal Umroh";
?>
...
@media print {
.bgkui {
      background:url(../images/bgkui.jpg) !important;
...
function printKui(){
	var vSuccess = /kuisuccess/g;
    var vLastKui = $('#hKui').val();
	if (confirm('Dengan mencetak kuitansi ini, maka akan menambah nomor kuitansi terakhir menjadi '+pad(vLastKui,4)+', apakah Anda yakin mencetak?')){
			var vURL = 'mpurpose_ajax.php?op=printkui&year=<?=date("Y")?>&month=<?=date("m")?>';
			var vKuiid = $('#hKui').val();
			
			$.post(vURL, {mem:'<?=$vJamaah?>',kuiid: vKuiid, payfor:'<?=$vPayFor?>', nom:'<?=$vAngsuran?>'}, function(data) {
...
$(document).ready(function(){
   ...
   $.post('mpurpose_ajax.php?op=countkui&year=<?=date("Y")?>&month=<?=date("m")?>',function(data){
	    var vNextKui = parseFloat(data.trim()) + 1;
	    
		$('#lastKui').html(pad(vNextKui,4));
		$('#hKui').val(vNextKui);
   });
});
...
<div >
<table   border=0 cellpadding=0 cellspacing=0 class="bgkui" style='width:800px;border-collapse:
...
">
 ...
 <tr height=25 style='height:18.75pt'>
  ...
  <td class=xl77 colspan=4 align=left style='mso-ignore:colspan;border-right:
  1.0pt solid black'>NO. KS <span id="lastKui" style="color:red"></span><?=date("/m/Y")?>/AMINAH TOUR <input type="hidden" id="hKui" name="hKui" value="" /></td>
 </tr>
 <tr height=20 style='height:15.0pt'>
  <td height="20" class=xl68>&nbsp;</td>
  <td></td>
  <td class=xl74 colspan=6 align=left style='mso-ignore:colspan'>Jl. Raya Wiguna Timur 48A Surabaya</td>
...
">
 ...
 <tr height=25 style='height:18.75pt'>
  <td height="25" class=xl68>&nbsp;</td>
  <td></td>
  <td colspan=4 class="xl78" bgcolor="#CCCCCC" ><div style="border: 1px solid #666;display:block;text-align:right;vertical-align:middle;padding:5px 5px 5px 5px">Rp <?=number_format($vAngsuran,0,",",".")?></div></td>
 ...
 <tr height=0 style='display:none'>
  <td width=74 style='width:54pt'></td>
  <td width=4 style='width:54pt'></td>
  <td width=68 style='width:54pt'></td>
  <td width=88 style='width:69pt'></td>
  <td width=68 style='width:54pt'></td>
  <td width=68 style='width:54pt'></td>
  <td width=68 style='width:54pt'></td>
  <td width=70 style='width:54pt'></td>
  <td width=11 style='width:54pt'></td>
  <td width=50 style='width:54pt'></td>
  <td width=74 style='width:54pt'></td>
  <td width=78 style='width:54pt'></td>
  <td width=14 style='width:54pt'></td>
 </tr>
 <![endif]>
</table>
</div>
