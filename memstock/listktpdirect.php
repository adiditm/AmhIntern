...
if ($_GET['tfDepart'] !='') 
 ...
 $vSQL="select a.* from m_korwil a left join m_admin b on a.fidkorwil=b.fkorwil where b.fidmember='$vUser'";
 $db->query($vSQL);
 $db->next_record();
 $vLevel = $db->f('flevel');
...
$vsql="select * from m_anggota a where concat(a.fprop,a.fkota,a.fkec) in (select concat(a.fprop,a.fkabkota,a.fkec) as farea from tb_korwil_area a left join m_korwil 
b on a.fidkorwil=b.fidkorwil where a.fidkorwil='$vUser' ) ";
...
*/
	...
	<div class="right_col" role="main">
		...
		<div><label>
		<h3>Daftar Jamaah  di Area  
	  <? if ($vLevel=='SUBKOR') echo 'Sub'; ?> Korwil <?=$vUser?></h3></label></div>
		  ...
		  <table border="0" cellpadding="4" cellspacing="0"  align="left" >
            ...
            <tr>
              ...
              <td colspan="2"><div align="left"><br>
                ...
                <input name="Submit2" type="button" class="btn btn-default" value="Reset" onClick="document.location.href='../memstock/listreg.php?op=&current=mdm_korwil_sub&menu=mdm_korwil_sublistreg'" />
    ...
    <table width="100%" border="0" align="left" cellpadding="1" cellspacing="0" class="table" style="background-color:white" >
      ...
      <tr style="color:;font-weight:bold"> 
        ...
        <td width="268"><div align="center" > Tanggal Daftar</div></td>
		 ...
		 <td width="150" align="center">Tanggal Pelunasan</td>
        ...
        <td width="150" class=""><div align="center">Paket - Program</div></td>
        <td width="633" align="center">Status</td>
...
b on a.fidkorwil=b.fidkorwil where a.fidkorwil='$vUser' )  ";
  ...
  <td  style="height: 39px" nowrap> 
          ...
          <span >
            ...
            <?=$oPhpdate->YMD2DMY($db->f('ftgldepart'))?>
  ...
  <td  style="height: 39px" nowrap>&nbsp;
  <?
  	 $vSQL = "select min(ftanggal) as ftgllunas from tb_payhist where fidmember='".$db->f('fidmember')."' and (fkind='flunas' or fkind='lunas')";
	$dbin->query($vSQL);
			$dbin->next_record();
			echo $oPhpdate->YMD2DMY($dbin->f('ftgllunas'));
...
<ul class="pagination" >
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($idisp!=$vPage) {
?>
              <li ><a  href="listktpdirect.php?lmAktif=<?=$vAktif?>&lmMship=<?=$vPrem?>&uPage=<?=$idisp?>&lmSort=<?=$vSort?>&current=<?=$_REQUEST['current']?>&uRegistrar=<?=$vUser?>" >
              <?=$i+1?>
              </a> </li> 
              <?
  } else {
