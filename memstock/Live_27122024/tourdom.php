...
<style type="text/css">
  ...
  <div><label>
  <h3>Booking Tour Domestik</h3></label></div>
   ...
   if ($vHari !='')
    ...
    $vSQL="select * from m_tour where fstatusrow=1 and ftgldepart >= date(now()) and date(fexpired) >= date(now()) and fgroup='d'  $vAnd order by fcountry";
   $db->query($vSQL);
   ...
   $pagenum=ceil($rows/$rowpage);
       if ($vScript=='loggedin.php') {
		  $vDetLink="../memstock/tourdet.php&type=TD";
		  $vClear="?tack=tour";
	   ...
	   $vDetLink="../memstock/tourdet.php?type=TD&$vString";
...
<input class="btn btn-success" name="btCheckHrg" type="button" id="btCheckHrg" value="Search" onclick="demoform.submit()" />
             &nbsp; <input  class="btn btn-default" name="btCheckHrg2" type="button" id="btCheckHrg2"  value="Clear Search" onclick="document.location.href='../memstock/tourdom.php?op=&current=mdm_transaction&menu=mdm_trans_tourdom'" />
</div>
        </div>
...
<div class="row">
 ...
 <div class="col-lg-6 hide"><label>Jumlah Hari </label>
...
<div class="row"> 
  ...
  while ($db->next_record()) {
	 ...
	 $vDesc=$db->f('fdesc');
	 $vDet=$db->f('fdetail');
	 $vDepart=$oPhpdate->YMD2DMY($db->f('ftgldepart'));
