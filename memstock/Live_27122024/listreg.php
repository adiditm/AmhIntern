...
$vsql="select * from m_anggota where 1 and fidregistrar ='{$_SESSION['LoginUser']}' ";
...
*/
	...
	<div class="right_col" role="main">
		...
		<div><label><h3>Daftar Jamaah yang Didaftarkan oleh <? if ($vLevel=='SUBKOR') echo 'Sub'; ?> Korwil <?=$_SESSION['LoginUser']?></h3></label></div>
...
<form name="memberForm">
  ...
  <td  style="height: 39px" nowrap>&nbsp;
  <?
  	 $vSQL = "select min(ftanggal) as ftgllunas from tb_payhist where fidmember='".$db->f('fidmember')."' and fkind='flunas'";
	$dbin->query($vSQL);
			$dbin->next_record();
...
<ul class="pagination" >
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($idisp!=$vPage) {
?>
              <li ><a  href="listreg.php?lmAktif=<?=$vAktif?>&lmMship=<?=$vPrem?>&uPage=<?=$idisp?>&lmSort=<?=$vSort?>&current=<?=$_REQUEST['current']?>" >
              <?=$i+1?>
              </a> </li> 
              <?
  } else {
