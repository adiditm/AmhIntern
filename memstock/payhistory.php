<? 
 ...
 if ($_GET['uMemberId']=='') $_GET['uMemberId'] = $_SESSION['LoginUser'];
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_payhist where fkind in('sawal','angs1','angs2','angs3','angs4','flunas','sawal','assure','arabassure','handle','voucher') and fidmember='$vUserChoosed' $vCrit";
...
<div class="right_col" role="main">
		...
		<div><label>
		<h3>History Pembayaran <?=$vRefUser." / ".$oMember->getMemberName($vRefUser);?></h3></label></div>  		       
...
<br>
        ...
        <tr style="font-weight:bold">
          ...
          <td width="13%" align="center" class="hide">Debit</td>
        ...
        <?
			    ...
			    $vSQL="select * from tb_payhist where   fkind in('fstorawal','fangsur1','fangsur2','fangsur3','fangsur4','flunas','sawal','assure','arabassure','handle','voucher') and fidmember='$vUserChoosed'   ";
...
<?=number_format($vBal,0,",",".");?>
        ...
        <tr class="tbrekapfont">
          ...
          <strong><td align="right"><?=number_format($vTotPay,0,",",".");?></td></strong>
          <td colspan="2" align="center" class="hide">&nbsp;</td>
          </tr>
		
		<? if ($db->num_rows() <=0){ ?>
		      <tr class="tbrekapfont">
          <td colspan="6"  align="center">Tidak ada history, atau pembayaran Anda belum disetujui!</td>
          </tr>
		  <? } ?>
      </table>
