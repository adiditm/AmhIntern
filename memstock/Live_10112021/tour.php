...
<style type="text/css">
  ...
  <div><label>
  <h3>Booking Tour Internasional</h3></label></div>
 ...
 //     $vAnd.=" and fpaket=$vPack ";
   ...
   if ($vHari !='')
    ...
    $vSQL="select * from m_tour where fstatusrow=1 and fgroup='t'  $vAnd order by fcountry";
   $db->query($vSQL);
   ...
   $pagenum=ceil($rows/$rowpage);
       if ($vScript=='loggedin.php') {
		  $vDetLink="../memstock/tourdet.php&type=TI";
		  $vClear="?tack=tour";
	   } else  if ($vScript=='index.php') {
		  $vDetLink="?tick=tourdet&type=TI";
		  $vClear="?tick=tour";
	   }		
	   
	   $vDetLink="../memstock/tourdet.php?type=TI&$vString";
...
<input class="btn btn-success" name="btCheckHrg" type="button" id="btCheckHrg" value="Search" onclick="demoform.submit()" />
             &nbsp; <input  class="btn btn-default" name="btCheckHrg2" type="button" id="btCheckHrg2"  value="Clear Search" onclick="document.location.href='../memstock/tour.php?op=&current=mdm_transaction&menu=mdm_trans_tourint'" />
</div>
        </div>
