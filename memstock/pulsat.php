 ...
 include_once("classes/pulsaclass.php");
  ...
  $vArrSMS=array('TI5', 'TI10', 'TI25', 'TI50','TI100');
    ...
    if ($_POST['hPost']=='1') {
	 //   $oSystem->jsLocation('loggedin.php?tack=pulsat');
		//print_r($_POST);
		$vStatus='1';
...
//Temporary
				  ...
				  $vURLServer="http://windows.amhtechno.com/reqterminal.php?kprod=$vKdProd&msis=$vMsisdn&trxid=$vNextID&sig=$vSig";
				//exit;
                ...
                <tr>
                  ...
                  <td width="501"><label for="select"></label>
                    <select class="cbProd" onchange="changeTRX(this)" name="cbProd1" id="cbProd1">
                      ...
                      <?
                         $vSQL="select * from m_voucherhp where Brand in ('Simpati') and status='OK' order by Nom";
						 $db->query($vSQL);
...
$vHargaP=$db->f('Harga') + $vFeePublic;
					  ...
					  ?>
                      <option value="<?="$vValue|$vNominal|$vHarga|$vHargaAsli|$vBrand"?>"><?=$vLabel?></option>
                      <? } ?>
                  </select></td>
