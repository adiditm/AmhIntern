...
// $vInvest=$oMember->checkInvest($vUser);
 ...
 if ($oMember->getSaldoAdm($vUser,$vJenis) < $vNom) {
	$oSystem->jsAlert("Saldo tidak cukup untuk transfer!"); 
	$oSystem->jsLocation("../memstock/transfer.php?op=&current=mdm_pebisnis&menu=spon_trans_tsaldo");
	exit;
 }	 
   if ($vNom>0 ) {
...
function saveWithdraw() {
	...
	if (parseFloat(vWD) > parseFloat(vSaldoX)) {
		alert('Jumlah transfer '+vWD+' tidak boleh melebihi saldo '+vSaldoX);
		return false;
	}

	if (parseFloat(vWD) <=0 ) {
		alert('Jumlah transfer tidak boleh atau negatif');
		return false;
	}
