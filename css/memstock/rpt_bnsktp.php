...
$vCrit.=" and a.fdfrom = '$vFrom' and a.fdto = '$vTo'" ;
 ...
 $vsql="select a.* from tb_komisi a left join m_korwil b on a.fidkorwil=b.fidkorwil left join m_admin c on a.fidkorwil=c.fkorwil where a.fkind='KTP' and c.fidmember='$vUser' ";
        ...
        <div class="row"> 
           ...
           <b>  Periode </b>  </div>
              <div class="col-lg-4">
               ...
               <?
               		 $vSQL="select distinct concat(a.fdfrom,'_',a.fdto) as fperiod, fdfrom, fdto from tb_komisi a left join m_admin b on a.fidkorwil=b.fkorwil where a.fkind='KTP' and b.fidmember='$vUser' ";
					$dbin->query($vSQL);
...
<br />
    ...
    <div class="table-responsive">
        <table width="90%" border="0" class="table table-striped">
          <tr >
            ...
            <td width="15%" align="center" style="height: 24px" class=""><strong>Status</strong></td>
            </tr>
          <? 
             $vNo=0;
			 ...
			 $vsql ="select a.*, b.fnama from tb_komisi a left join m_korwil b on a.fidkorwil=b.fidkorwil left join m_admin c on a.fidkorwil=c.fkorwil where a.fkind='KTP' and c.fidmember='$vUser' ";
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($i!=$vPage) {
?>
          <li class="active" ><a href="../manager/rpt_bnsktp.php?uPage=<?=$i?>&uAwal=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAwal,"-"),"-")?>&uAkhir=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAkhir,"-"),"-")?>" >
          <?=$i+1?>
          </a></li>
          <?
  } else {
