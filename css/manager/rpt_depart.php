        ...
        <div class="row hide"> 
           ...
           <b>  Tgl. Keberangkatan </b>  </div>
              <div class="col-lg-2">
               ...
               <?
               		$vSQL="select distinct ftgldepart from m_anggota where ftgldepart <> '' '0000-00-00' order by ftgldepart ";
					$dbin->query($vSQL);
  ...
  <tbody>      
         <?
					...
					while($dbin->next_record()) {
						...
						$vSQLAllJam = "select count(fidmember) as fjml from m_anggota where ftgldepart='$vTglDepart'";
						$db->query($vSQLAllJam);
						$db->next_record();
						$vAllJam = $db->f('fjml');
						
						$vSQLSiap = "select count(*) as fsiap from m_anggota where 1 $vAndBring  $vAndDoc  and ftotalbayar >= fprice ";
						$db->query($vSQLSiap);
						$db->next_record();
						$vSiap = $db->f('fsiap');
