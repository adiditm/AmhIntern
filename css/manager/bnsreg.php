...
<div class="row">
		<div><label>
		<h3>Bonus Registrasi</h3></label></div>  		       
...
<tr style="font-weight:bold">
  ...
  <td  width="167" align="center">Bonus per Registrasi</td>
						...
						while($db1->next_record()) {
							...
							while($dbin1->next_record()) {
								$vIDProg = $dbin1->f('fidprogram');
										...
										// $vSQLUserSub = "select fidmember from m_admin where fkorwil='$vIDKorwil'";
										 $vSQLReg = "select count(fidmember) as fjml from m_anggota where (fidregistrar in ($vSQLUserSpon) or fidregistrar in($vSQLUserOldSpon)) and flunas > 0 and fprogram='$vIDProg' and fpaket='$vIDPaket' $vAnd ";
		  ...
		  <?  
						...
						while($db1->next_record()) {
								$vIDPaket = $db1->f('fpackid');
									...
									while($dbin1->next_record()) {
								 	...
								 	$vSQLUser = "select fidmember from m_admin where fkorwil='$vIDKorwil'";
									 $vSQLKTP = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUser)  and fprogram='$vIDProg' and flunas >0 and  fpaket='$vIDPaket' $vAnd ";
						...
						while($db1->next_record()) {
								$vIDPaket = $db1->f('fpackid');
									...
									$vSQLKTP = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUserSub)  and fprogram='$vIDProg' and flunas >0 and  fpaket='$vIDPaket' $vAnd ";
						...
						while($db1->next_record()) {
								 ...
								 $vSQLUserSpon ="select a.fidmember from m_admin a left join m_anggota b on a.fidjamaah=b.fidmember where concat(b.fprop,b.fkota,b.fkec)  in($vSQLArea)";
											...
											$vSQLKTP = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUserSpon)  and fprogram='$vIDProg' and flunas >0 and  fpaket='$vIDPaket' $vAnd ";
