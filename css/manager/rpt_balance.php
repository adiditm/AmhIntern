...
<form name="memberForm">
    ...
    <table width="100%" border="0" align="left" cellpadding="1" cellspacing="0" class="table" style="background-color:white" >
      ...
      <tr style="color:;font-weight:bold"> 
        ...
        <td width="43" style="display:none"><div align="left" >&radic;</div></td>
 ...
 <? if($vAktifList=='1') { ?>	
        ...
        <td class="hide"  style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>"><div align="right">
          ...
          <?=number_format($vSaldoAcc,0,",",".")?>
              ...
              <td  style="height: 39px;display:none" <? if($vFall==$db->f('fidkorwil')) echo "bgcolor=#0f0";?> ><span >
...
<ul class="pagination" >
   ...
   for ($i=0;$i<$vPageCount;$i++) {
	...
	// if ($vOP=="post") $idisp=0;
     if ($idisp!=$vPage) {
?>
              <li ><a  href="rpt_balance.php?lmAktif=<?=$vAktif?>&lmMship=<?=$vPrem?>&uPage=<?=$idisp?>&lmSort=<?=$vSort?>&current=<?=$_REQUEST['current']?>&tfID=<?=$vID?>&tfNama=<?=$vNama?>" >
              <?=$i+1?>
              </a> </li> 
              <?
  } else {
