...
<br />
    ...
    <div class="table-responsive">
        ...
        <thead>
          ...
          <tr id="tr<?=$vIdSys?>">
            ...
            <td id="tdstat<?=$vIdSys?>" class=""><input  id="btnPay<?=$vIdSys?>" name="btnPay<?=$vIdSys?>" type="button" class="btn btn-info btn-xs" value="Tandai Terbayar" onClick="setPay('<?=$vIdSys?>','<?=$vRemark?>')" <? if ($vStatPay=='1') echo 'disabled'; ?>></td>
            </tr>
           <? } ?>
          <tr class="hide">
            <td style="width: 5%" >&nbsp;</td>
   ...
   for ($i=0;$i<$vPageCount;$i++) {
     ...
     if ($i!=$vPage) {
?>
          <li class="active" ><a href="../manager/bnsregpost.php?uPage=<?=$i?>&uAwal=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAwal,"-"),"-")?>&uAkhir=<?=$oPhpdate->DMY2YMD($oPhpdate->YMD2DMY($vAkhir,"-"),"-")?>" >
          <?=$i+1?>
          </a></li>
          <?
  } else {
