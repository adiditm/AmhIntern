...
if ($vSort =='M')
   ...
   $vOrder.=" order by a.fdebit  ";
			 ...
			 $vsql="select a.*, b.*,c.fnamabank as cfnamabank,  date(a.ftanggal) as ftglcompile,  c.fkdbankreal as fkdreal from tb_kom_stockist a left join m_anggota b on a.fidmember=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.fkind='resetmonthst'"; 
...
//cek keberadaan total bonus
					   ...
					   if ($dbin->num_rows() <=0) {					   
						  ...
						  // 	echo $vMsg="<font color='#00f'>$vRows:Insert transfer Mandiri ($vUserName: $vSisa), Insert CF = 0 </font><br>";
						   ...
						   $vResult=$oEspay->transferFund($vSourceBankCode,$vSourceBankAcc,$vKodeBankReal,$vRek,$vNama,$vSisa,'Transfer Bonus Stockist Onotoko',$vUserName.'-'.date("Y-m-d H:i:s"));
						   echo "Result: ".$vResult."<br>";
						   if (trim($vResult) !='') {
							    $vResArr=json_decode($vResult,true);
								if ($vResArr['error_message']=='Success') {
								    $vSQL="update tb_cftrans_smost set ffeestatus='2', fpaylog='$vResult' where fidmember='$vUserName' and date(ftanggal) = '$vAkhir'";
									$db1->query($vSQL);
									$db1->query("update tb_mutasi set fstatus='2' where fidmember='$vUserName' and fkind in('resetmonthst') and date(ftanggal) = '$vAkhir' and fstatus='1' ");
								}
  ...
  $mtime = microtime(); 
   ...
   echo $vMsg="</body></html>";
   $vMsgAll .= $vMsg;
   			 $oSystem->smtpmailer($oRules->getSettingByField('fmailbcc'),$vMailFrom,'Onotoko','Compile Transfer Bonus Stockist Bulanan','Transfer Bonus Stockist Bulanan Compiled!',$oRules->getSettingByField('fmailbcc'),"");
