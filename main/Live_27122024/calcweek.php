...
//echo $vSQLin."<br>";
			...
			$db->query($vSQLin);
		    $db->next_record();
	    	echo $vMsg="<font color='#0f0'>Hitung bonus mingguan (Presenter) sebelum $vDateCompile : " ;
			$vMsgAll .= $vMsg;
    ...
    $oSystem->smtpmailer($oRules->getSettingByField('fmailbcc'),$vMailFrom,'Onotoko','Compile Mingguan A (Presenter)','Well Run!',$oRules->getSettingByField('fmailbcc'),"");
   $mtime = microtime(); 
   $mtime = explode(" ",$mtime); 
   $mtime = $mtime[1] + $mtime[0]; 
   $endtime = $mtime; 
