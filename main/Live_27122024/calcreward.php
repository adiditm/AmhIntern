...
$vMsg="";
 ...
 //  $vMsg.="==================================================================================================";
		 ...
		 while ($db1->next_record()) {
			...
			if ($vFulFill == "Ya") {
					...
					if ($db->num_rows() <=0) {
					 ...
					 $vMemName = $oMember->getMemberName($vUser);
				 $vIsiSMS=str_replace("{REWARD}",number_format($vBonus,0,",","."),$vIsiSMS);
				  $vIsiSMS=str_replace("{MEMID}",number_format($vUser,0,",","."),$vIsiSMS);
				   $vIsiSMS=str_replace("{MEMNAME}",number_format($vMemName,0,",","."),$vIsiSMS);
				 if ($vHP !='')
				     $oSystem->sendSMS($vHP, $vIsiSMS,$vUserGO,$vPassGO);
					 
				$vSubject="Pemberitahuan  Bonus Reward";
				 if ($vEmail !='')
				     $oSystem->smtpmailer($vEmail,$vMailFrom,'Onotoko',$vSubject,$vIsiSMS,$oRules->getSettingByField('fmailbcc'),"");
  ...
  $oSystem->smtpmailer($oRules->getSettingByField('fmailbcc'),$vMailFrom,'Onotoko','Compile Reward AB','Well Run!',$oRules->getSettingByField('fmailbcc'),"");
