...
//cek keberadaan total bonus
					   ...
					   if ($dbin->num_rows() <=0) {					   
						   ...
						   $vResult=$oEspay->transferFund($vSourceBankCode,$vSourceBankAcc,$vKodeBankReal,$vRek,$vNama,$vSisa,'Transfer Bonus Harian Onotoko',$vUserName.'-'.date("Y-m-d H:i:s"));
						   echo $vResult."<br>";
						   if (trim($vResult) !='') {
								...
								if ($vResArr['error_message']=='Success') {
									...
									$vSQL="update tb_cftrans_sday  fpaylog='$vResult' where fidmember='$vUserName' and date(ftanggal) = '$vAkhir'";
									$db1->query($vSQL);								
 ...
 $mtime = microtime(); 
  ...
  // echo "Total time  ".$totaltime." seconds"; 
   ...
   $vMsgAll .= $vMsg;	
   			 ...
   			 $oSystem->smtpmailer($oRules->getSettingByField('fmailbcc'),$vMailFrom,'Onotoko','Compile Transfer Bonus Harian','Transfer Bonus Harian Compiled!',$oRules->getSettingByField('fmailbcc'),"");
