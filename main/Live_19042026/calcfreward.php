...
$vMsgAll.="<html><head><title>Compile (CF) Reward </title></head><body>";
...
if ($vSort =='M')
   ...
   $vOrder.=" order by a.fdebit  ";
			 ...
			 $vsql="select a.*, b.*,c.fnamabank as cfnamabank,  date(a.ftanggal) as ftglcompile, c.fkdbankreal as fkdreal, a.fidsys as asys from tb_rewards a left join m_anggota b on a.fkdanggota=b.fidmember left join m_bank c on b.fnamabank=c.fkodebank where a.fpaid='0'"; 
		...
		//	  echo $vCount=$db->num_rows();
		        ...
		        while ($db->next_record()) {
				   $vUserName=$db->f('fkdanggota');
				   $vBonus=$db->f('fnominal');
				   $vASys=$db->f('asys');
				   $vBonusOri=$vBonus;
 ...
 //Ambil CF kemarin
				   $vSQL="select fidsys,fcftrans,ftanggal from tb_cftransrwd where fidmember='$vUserName' and date(ftanggal) < date('$vAkhir') and ftype='cfrwd' and ffeestatus='1'";
				   $dbin->query($vSQL);
				   $dbin->next_record();
				   $vCF=$dbin->f('fcftrans');
...
//cek keberadaan total bonus
					   $vSQL="select * from tb_cftrans_srwd where fidmember='$vUserName' and date(ftanggal) =  date('$vAkhir') and ftype='cfsrwd';";
					   $dbin->query($vSQL);
					   $dbin->next_record();
					   if ($dbin->num_rows() <=0) {					   
						     $vSQL="insert into tb_cftrans_srwd(fidmember, ftglbefore,fcfbefore,fbonusnow,ftanggal, fcftrans, ftype) ";
						   ...
						   $vSQL="insert into tb_cftransrwd(fidmember, ftglbefore,fcfbefore,fbonusnow,ftanggal, fcftrans, ftype, flastop, ffeestatus) ";
						   ...
						   $vResult=$oEspay->transferFund($vSourceBankCode,$vSourceBankAcc,$vKodeBankReal,$vRek,$vNama,$vSisa,'Transfer Bonus Reward Onotoko',$vUserName.'-'.date("Y-m-d H:i:s"));
						 //  echo $vResult."<br>";
						   if (trim($vResult) !='') {
							    $vResArr=json_decode($vResult,true);
								if ($vResArr['error_message']=='Success') {
								    $vSQL="update tb_cftrans_srwd set ffeestatus='2', fpaylog='$vResult' where fidmember='$vUserName' and date(ftanggal) = '$vAkhir'";
									$db1->query($vSQL);
									
								    $vSQL="update tb_rewards set fpaid='1', ftglpaid='$vAkhir' where fidsys='$vASys' ";
									$db1->query($vSQL);
								} else {
									$vSQL="update tb_cftrans_rwd  fpaylog='$vResult' where fidmember='$vUserName' and date(ftanggal) = '$vAkhir'";
									$db1->query($vSQL);								
						   ...
						   if ($vCF >0)  {
								   echo $vSQL="update tb_cftransrwd set ffeestatus='0', flastop='transupd' where fidsys=$vIdSys";
								   $db1->query($vSQL);	
								   echo "<br>";		
						   }
							echo "<br>";
					   ...
					   $vSQL="select * from tb_cftransrwd where fidmember='$vUserName' and date(ftanggal) =  date('$vAkhir') and ftype='cfday';";
					  // echo "<br>:";
					   $dbin->query($vSQL);
					   $dbin->next_record();
					  // echo $dbin->num_rows();
					   if ($dbin->num_rows() <=0) {
						   $vSQL="insert into tb_cftransrwd(fidmember, ftglbefore,fcfbefore,fbonusnow,ftanggal, fcftrans, ftype) ";
						...
						if ($vCF >0)  {
								   $vSQL="update tb_cftransrwd set ffeestatus='0',flastop='cfupd' where fidsys=$vIdSys";
								   $db1->query($vSQL);	
 ...
 $mtime = microtime(); 
  ...
  // echo "Total time  ".$totaltime." seconds"; 
   ...
   $vMsgAll .= $vMsg;	
   			 ...
   			 $oSystem->smtpmailer($oRules->getSettingByField('fmailbcc'),$vMailFrom,'Onotoko','Compile Transfer Bonus Reward','Transfer Bonus Reward Compiled!',$oRules->getSettingByField('fmailbcc'),"");
  // mail("a_didit_m@yahoo.com","Bonus Pairing Compilation $vNow",$vMsg);
   $vFileName='../files/CompileTransReward'.date('Y-m-d_H.i.s').'.htm';
