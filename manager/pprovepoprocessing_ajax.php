...
//ffeetrxstmob
	  ...
	  if($dbin->query($vSQL)) {
         ...
         $db->query("COMMIT;");
		  ...
		  $vSQL="select * from m_infodep  where ftgldepart='$ftgldepart' ";
		  $db->query($vSQL);
		  $db->next_record();
		...
		///Pembayaran Poin
			...
			//print_r($_POST);
			 if (trim($vMember) )  {
					...
					$vSQLHist = "INSERT INTO tb_payhist(fidmember, fidfunder, ftanggal, fdesc, fcredit, fdebit, fbalance, fkind, fstatus, flastuser, flastupdate, fref, fincometax, flog, fadminfee, fvat) 
					
					select fidmember, fidmember, ftglentry, fketerangan, fsubtotal,0,fsubtotal,fidproduk,fprocessed, '".$_SESSION['LoginUser']."',now(),fidpenjualan,0,'',0,0 from tb_payment_temp where fidpenjualan='$vIdTrx'; ";
					$db->query($vSQLHist);
					
					$vSQLUpdate = "update m_anggota set fangsur1 = (select fsubtotal from tb_payment_temp where fidpenjualan='$vIdTrx' ) where fidmember = (select fidmember from tb_payment_temp where fidpenjualan='$vIdTrx' ) ";
