<?
  ...
  if ($vReverCancel==$vCancel) {//Cancel
       ...
       // echo "$vIDJual,$vAdmin,$vFrom,$vNom, $vKet, $vTo";
 ...
 //   $oJual->processPay($vIDJual,$vAdmin);
	...
	if ($vProcess==1) {
        ...
        $vIsiAct="Transfer Saldo  $vIDJual Anda sudah diproses";
        $vMessage="$vNama, $vIsiAct  \n\n";
          $vMessage.="Terima kasih .\n\n";
        $vSMTP=$oRules->getSettingByField('fsmtp');
        $oSystem->sendMail($vFrom,$vEmail,$vNama,$oRules->getMailBCC(1),$oRules->getSubjAct(1),$vMessage,$vSMTP,"Admin"); 
      ...
      //	$oSystem->smsGateway(date("Y-m-d H:i:s"),preg_replace("/^0/","62",$vNoHP),$vMesgSMS,'Aktifasi Investasi');	
        ...
        $oSystem->jsAlert("Transfer saldo $vIDJual processed!.");
   } else $oSystem->jsAlert("Transfer saldo gagal!.");
	$oSystem->jsLocation("../manager/appvtrans.php?op=&current=mdm_admin&menu=mdm_admin_appvwith");
