...
function generateRandomString($length = 6) {
   ...
   if ($_POST['hPost'] != '1') {
	 ...
	 $oSystem->smtpmailer('japri_s@yahoo.com',$vMailFrom,'AMH',"Tabungan AMH Internal",'','',false);
	  ...
	  $vhSubTot=0;	  	 
		 ...
		 $vYear = date("Y");
		 $vMonth = date("m");
		 $vCounter = $oMember->getTabCount($vYear,$vMonth);
		 $vCounter++;
		 $vCounterStr = str_pad($vCounter,4,"0",STR_PAD_LEFT);
		 $vPrefix = "TAB".$fjenis."-".date("Ym");  	
		...
		$vFileNameKTP = str_replace("'","''",$vFileNameKTP);
		

			 if ($oMember->tabPilgrims($fidmember, $fjenis, $fstorawal, $fangsur1, $fangsur2, $fangsur3, $fangsur4, $flunas, $ftotalbayar, $frefer, $fnama, $tfHP, $fnamabank, $fnorekening, $ftempat, $ftgllahir, $falamat, $fkota, $fkodepos, $fprop, $femail, $fpassword, $fpaspor, $fnamarefer, $faktif, $ftglaktif, $fatasnama, $fkotabank, $ftgldaftar,  $ftglentry, $fcount, $fcabbank, $fcountrybank, $fswift, $flastuser, $flastupdate, $ftitle, $fsex, $fnation, $ffoto, $fdoc, $fnoktp, $fcountry, $fket, $fnpwp,$fpaket,$fpaketday,$fjenpay,$fprogram,$fprice,$fairporttax,$fassure,$fkakek,$fayah,$fibu,$fkec,$fdes,$fnohprefer,$fnoktprefer,$fnamabuss,$fnohpbuss,$fnoktpbuss,$ftgldepart,$farabassure,$fwarisbuss,$fpromo,$_SESSION['LoginUser'],$vFileName,$vFileNameKTP,$db)==1) {  

			 }
			
		
		$oMember->updTabCount($vYear,$vMonth,$vCounter,$db);


	if($db->query('COMMIT;')) {	  
	   $oSystem->jsAlert("Entri tabungan sukses dengan ID $fidmember!");	
	//  $oSystem->jsLocation("../manager/register.php?op=&current=mdm_memnet&menu=mdm_memnet_profile");
	  $oSystem->jsLocation('../manager/aktiftab.php?op=&current=mdm_admin&menu=mdm_admin_tabungan');
	  exit;
	}  else {
...
$(document).ready(function(){
			...
			*/
			...
			//$('.year').css("max-width", "90px");
			 ...
			 // $.validator.messages.required = '<span style="color:red;font-weight:normal">This field is required!</span>';
			  ...
			  $('#frmReg input, #frmReg textarea,  #frmReg select, #frmReg checkbox, #frmReg radio').not([type="submit"]).not($("#fidmember")).not($("#femail")).not($("#fpropo")).not($("#fkota")).not($("#fpaspor")).not($("#tfBranchBank")).not($("#tfSernoUpName")).not($("#femail")).addClass('required');  
	...
	<? if ($vPaket !='') {?>
		...
		var uploadField = document.getElementById("fttd");
		
		uploadField.onchange = function() {
			if(this.files[0].size > 1024000){
			   alert("File tanda tangan tidak boleh melebihi 1MB!");
			   this.value = "";
			};
		};
		
		
		var uploadField = document.getElementById("fktpdoc");
		
		uploadField.onchange = function() {
			if(this.files[0].size > 1024000){
			   alert("File KTP tidak boleh melebihi 1MB!");
			   this.value = "";
			};
		};		
...
<? } ?>
 ...
 <div class="right_col" role="main">
		...
		<div><label>
		<h3>Form Tabungan Jamaah <? if ($_GET['current']=='mdm_korwil_sub') echo " oleh Korwil / Sub Korwil";?></h3></label></div> 
...
<div class="divtr col-lg-6" >
								...
								Nama Ibu*</label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-user"></i></span>
                                <input type="text" class="form-control" id="fibu" name="fibu" placeholder="(Tanpa tanda koma)*" onBlur="" onKeyUp="">
                                </div>
</div> 
...
<div class="divtr col-lg-6">
                                  <label for="exampleInputEmail1" >Tanggal Berangkat*  </label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-calendar"></i></span>
                                          <select class="form-control m-bot15" id="ftgldepart" name="ftgldepart" onChange="$('#lblStor').remove();spreadPProg(this.value,$('#fprogram').val(),$('#fpaket').val());getSeat(this.value)" onFocus="$('#divSeat').hide();" onClick="$('#divSeat').hide();" disabled>
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
								<? 
...
<div class="col-md-5 col-lg-6 divtr" >
                                 ...
                                 <select class="form-control m-bot15" id="fpaket" name="fpaket" onChange="spreadPProg($('#ftgldepart').val(),$('#fprogram').val(),this.value)" disabled>
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
								<? 
...
<div class="col-md-5 col-lg-6 divtr" id="divProg" >
                            ...
                            <select class="form-control m-bot15" id="fprogram" name="fprogram" onChange="spreadPProg($('#ftgldepart').val(),this.value,$('#fpaket').val())" disabled>
                            <option  value="" selected="selected" >--Pilih / Choose--</option>
                                <?
...
<div class="col-md-5 col-lg-12 divtr " id="divPromo" style="display:none">
                          ...
                          <div style="" class=" divtr col-lg-6">
							...
							<label for="exampleInputEmail1">Jumlah Hari* <div align="left" style="display:inline" id="statKitPres"></div></label>
                                    ...
                                    <input type="text" class="form-control" id="fpaketday" name="fpaketday" placeholder="Masukkan Jumlah Hari" value="0" <?=$vRead?> onBlur=""  readonly >
                                    </div>                            
                            </div>   
...
<div style="" class=" divtr col-lg-6">
 ...
 <div style="" class=" divtr col-lg-6">
							<label for="exampleInputEmail1">Asuransi Arab Saudi* <div align="left" style="display:inline" id="statKitUp"></div></label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-money"></i></span>
                                    <input type="text" class="form-control" id="farabassure" name="farabassure" placeholder="0" value="0"  onBlur="" dir="rtl" onKeyUp="if (!isNaN(this.value) && this.value !='') calcPay();" readonly   >
                                    </div>                            
 </div>
...
<div style="" class=" divtr col-lg-6">
    ...
    <div style="" class=" divtr col-lg-6">
							...
							<label for="tfSernoSponName">Tanda Tangan </label>
                                      ...
                                      <input type="file"  class="form-control" id="fttd" name="fttd" placeholder="Scan / foto tanda tangan" value="0"  dir="rtl" readonly accept="image/png, image/gif, image/jpeg , image/jpg"   >
