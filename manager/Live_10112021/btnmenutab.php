<div class="row" style="text-align:left">
  ...
  <? if ($vCurrent=='mdm_admin') { ?>      
          <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_genealogi")) { ?> <input alt="Detail / Verifikasi Data" name="btDetail<?=$vSeq?>" type="button" class="btn btn-success btn-sm" id="btDetail<?=$vSeq?>" onClick="return MM_goToURL('parent','../manager/profiletab.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue());return document.MM_returnValue" value="Detail / Verifikasi Data &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? }  ?>        
