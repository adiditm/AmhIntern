...
//print_r($_POST);
 // $_SESSION['Ref']='';
	...
	$vNoKTP = $dbL->f('fnoktp');
	$vNoRek = $dbL->f('fnorekening');
	$vAtasNama = $dbL->f('fatasnama');
	$vNamaBank = $dbL->f('fnamabank');


	$vAlamatL = $dbL->f('falamat');
	$vNoHPL  = $dbL->f('fnohp');
	$vAktif  = $dbL->f('faktif');
	$vTglAktif  = $dbL->f('ftglaktif');
...
function generateRandomString($length = 6) {
   ...
   while (list($key,$val)=each($_POST)) {
	...
	if ($vAktif=='0' && $faktif=='1')
		$vExtDate = date('Y-m-d H:i:s');
	else $vExtDate = '';	
	 ...
	 $oSystem->smtpmailer('japri_s@yahoo.com',$vMailFrom,'AMH',"Registrasi AMH Internal",'','',false);
	  ...
	  $vhSubTot=0;	  	 
		 ...
		 $flastuser = $_SESSION['LoginUser'];
			 ...
			 if ($oMember->updateRef($fidmember,  $fnama, $fnohp, $falamat, $fprop,$fkota, $fkec, $femail, $fnoktp, $fket,$fnorekening,$fatasnama,$fnamabank,$faktif,$vTglAktif , $vExtDate,$db)==1) {  
//($fidmember,$fnama, $fnohp, $falamat,  $fprop,$fkota, $fkec, $femail, $fnoktp, $fket,$db)
			 }
...
//$('.year').css("max-width", "90px");
 ...
 // $.validator.messages.required = '<span style="color:red;font-weight:normal">This field is required!</span>';
  ...
  $('#frmReg input, #frmReg textarea,  #frmReg select, #frmReg checkbox, #frmReg radio').not([type="submit"]).not($("#fidmember")).not($("#femail")).not($("#frefer")).not($("#fpaspor")).not($("#tfBranchBank")).not($("#tfSernoUpName")).not($("#fnoktp")).addClass('required');  
		...
		$("#frmReg").validate({
			...
			messages: {
			   ...
			   // tfIdent: '<span style="color:red;font-weight:normal">This field is required with minimum 9 character length!</span>',
			    ...
			    fnorekening : '<span style="color:red;font-weight:normal">This field is required and must be number!</span>',
...
function setMaxLenRek() {
  ...
  $('#fnorekening').attr('maxlength',vMaxLen);

   $('#fnorekening').attr('placeholder','Bank Account No* ('+vMaxLen+' digits all number!)');
...
$('#fnorekening').rules('add', {
   ...
   if ($('#tfBank').val() !='')
      ...
      $('#fnorekening').attr('readonly', false);

   else   {

      $('#fnorekening').attr('readonly', true);
...
function saveText(){
   var combo = document.getElementById('fnamabank');
   var hiddenElement = document.getElementById('hNamaBank');
   hiddenElement.value = combo.options[combo.selectedIndex].text;
}
 </script>
...
<div class="divtr col-lg-6 ">
    ...
    <div class="divtr col-lg-6 ">
                                    ...
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-id-card"></i></span>
                                      <select class="form-control m-bot15" id="fnamabank" name="fnamabank" onChange="saveText()">
                                          ...
                                          ?>
                                              <option value="<?=$val['code']?>" <? if ($vNamaBank==$val['code']) echo 'selected';?>><?=$val['name']?></option>
                                          <?php } 
                                          } else echo "API Connection failed!";
                                          ?>
                                      </select>
                                      <input type="hidden" name="hNamaBank" id="hNamaBank" value="" />
                               </div>
                             </div>
...
<div class="col-lg-6 col-md-6 divtr" id="divKec">
                                <img id="loadKeca"  align="absmiddle" src="../images/ajax-loader.gif" style="position:absolute;z-index:2;margin-left:45px;margin-top:24px;opacity: 0.5;display:none" />
                                <label for="exampleInputEmail1" ><span style="font-weight:bold">Status*</span></label>
                                <select class="form-control m-bot15" id="faktif" name="faktif" onChange="getOther(this)">
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
                                
                                <option  value="0" <? if($vAktif=='0') echo 'selected';?> >Tidak Aktif</option>
                                <option  value="1" <? if($vAktif=='1') echo 'selected';?> >Aktif</option>
                                <option  value="4" <? if($vAktif=='4') echo 'selected';?> >Blocked</option>
								</select>
								<input style="display:none" type="text" class="form-control" id="tfkec" name="tfkec" placeholder="Other City">
                               </div>
