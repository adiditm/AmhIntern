...
//print_r($_FILES);
 // $_SESSION['Ref']='';
    ...
    $vSQL="select a.*, b.fpackname, c.fdesc, c.fplane, c.fhotel, d.fdesc as tourdesc, d.fplane as tourplane, d.fhotel as tourhotel, d.fjmlhari as tourday, c.fdaycount as promoday,c.fdepart as promodepart, d.ftgldepart as tourdepart, c.fplane as promoplane, d.fplane as tourplane, c.fhotel as promohotel, d.fhotel as promohotel, e.fnama as prognama from m_anggota a  left join m_paket b on a.fpaket = b.fpackid left join tb_promo c on a.fpromo=c.fidpromo left join m_tour d on a.fprod=d.fidtour left join m_program e on a.fprogram=e.fidprogram where a.fidmember='$vUserActive'";
...
function removeSpecChar(e) {
 ...
 <div class="right_col" role="main">
		...
		<div><label>
		<h3> Info Keberangkatan</h3></label></div> 
...
<form method="post" id="frmReg" name="frmReg" action="" enctype="multipart/form-data" >
  ...
  <tr>
    <td width="23%"><span style="font-weight: bold">ID Jamaah </span></td>
    <td width="77%">&nbsp;<?=$dbL->f('fidmember')?></td>
  </tr>
  <tr>
   ...
   <tr>
    <td><span style="font-weight: bold">Paket Umroh </span></td>
    <td>&nbsp;<?=$dbL->f('fpackname')?></td>
  </tr> 
  <tr>
    <td><span style="font-weight: bold">Program Umroh </span></td>
    <td>&nbsp;<?=$dbL->f('prognama')?></td>
  </tr>
  <tr>
    <td><span style="font-weight: bold">Jumlah Hari </span></td>
    <td>&nbsp;<? 
	
		if($dbL->f('tourday') != '')
		   echo $dbL->f('tourday');
		else if($dbL->f('fpaketday') != '')   
		    echo $dbL->f('fpaketday');
		else  echo $dbL->f('promoday');	 
	
	?></td>
  </tr>
  <tr>
    <td><span style="font-weight: bold">Tanggal Keberangkatan </span></td>
    <td>&nbsp;<? 
	
		if($dbL->f('tourdepart') != '')
		   echo $oPhpdate->YMD2DMY($dbL->f('tourdepart'));
		else if($dbL->f('promodepart') != '')   
		    echo $oPhpdate->YMD2DMY($dbL->f('promodepart'));
		else  echo $oPhpdate->YMD2DMY($dbL->f('ftgldepart'));	 
	
	?></td>
  </tr>
  <tr>
    <td><span style="font-weight: bold">Pesawat</span></td>
    <td>&nbsp;<? 
	
		if($dbL->f('tourplane') != '')
		   echo $dbL->f('tourplane');
		else if($dbL->f('promoplane') != '')   
		    echo $dbL->f('promoplane');
		else  echo $dbL->f('fplane');	 
	
	?></td>
  </tr>
  <tr>
    <td><span style="font-weight: bold">Hotel</span></td>
    <td>&nbsp;<? 
	
		if($dbL->f('tourhotel') != '')
		   echo $dbL->f('tourhotel');
		else if($dbL->f('promohotel') != '')   
		    echo $dbL->f('promohotel');
		else  echo $dbL->f('fhotel');	 
                     ...
                     <? } ?>
									...
									<? } ?>	
                                        <? if ($fidmember=='') $fidmember = $vUserActive;?>
                                         <button id="btnBack" type="button" class="btn btn-default"  onClick="document.location.href='../manager/indexnonadmin.php'">Back</button> <div id="divLoad" style="display:inline">
