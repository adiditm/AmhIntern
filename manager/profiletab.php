...
<? 
  ...
  if (preg_match("/aktiftab.php/",$vRefer)) {
...
//print_r($_POST);
 // $_SESSION['Ref']='';
    ...
    $vSQL="select a.* from m_anggota_tab a  where a.fidmember='$vUserActive'";
   ...
   if ($_POST['hPost'] != '1') {
	 ...
	 $oSystem->smtpmailer('japri_s@yahoo.com',$vMailFrom,'AMH',"Registrasi AMH Internal",'','',false);
	  ...
	  $vhSubTot=0;	  	 
		 ...
		 $vCurrSAwal = $oMember->getMemFieldTab('fstorawal',$fidmember); 
		 $vCurrAngs1 = $oMember->getMemFieldTab('fangsur1',$fidmember); 
		 $vCurrAngs2 = $oMember->getMemFieldTab('fangsur2',$fidmember); 
		 $vCurrAngs3 = $oMember->getMemFieldTab('fangsur3',$fidmember); 
		 $vCurrAngs4 = $oMember->getMemFieldTab('fangsur4',$fidmember); 
		 $vCurrLunas = $oMember->getMemFieldTab('flunas',$fidmember); 


		 $fdoc = $_FILES['fttd']['name'];
		 $path_info = pathinfo($fdoc);
		
		// print_r($path_info);
		// exit;
		 $vExt = $path_info['extension'];
		 $tmp_name = $_FILES['fttd']['tmp_name'];
		 $vLowName = strtolower($oMember->getMemFieldTab('fnama',$fidmember));
		 $vLowName = str_replace(" ","_",$vLowName);
		 
	 
		
		 if ($_FILES['fttd']['name'] !='') {
			     $vFileName = "../images/ttd/ttd-$fidmember-$vLowName".".$vExt"; 
				 $vTtd = ", fttd='$vFileName' ";
		 ...
		 $fktpdoc = $_FILES['fktpdoc']['name'];
		 $path_info = pathinfo($fktpdoc);
		
		// print_r($path_info);
		// exit;
		 $vExt = $path_info['extension'];
		 $tmp_name = $_FILES['fktpdoc']['tmp_name'];
		 $vLowName = strtolower($oMember->getMemFieldTab('fnama',$fidmember));
		 $vLowName = str_replace(" ","_",$vLowName);
		 		
		 if ($_FILES['fktpdoc']['name'] !='') {
			     $vFileNameKTP = "../images/ktp/ktp-$fidmember-$vLowName".".$vExt"; 
				 $fktpdoc = ", fktpdoc='$vFileNameKTP' ";
		 		if (file_exists($vFileNameKTP) && trim($fktpdoc) !='')
		     		unlink($vFileNameKTP);
		 		move_uploaded_file($tmp_name,$vFileNameKTP);
		 } else $fktpdoc='';
...
//echo "$fktpdoc :::: $vTtd";		 
	//	 print_r($_FILES);
		...
		// exit;
			 ...
			 if ($oMember->tabPilgrimsBasic($fidmember, $fjenis, $fstorawal, $fangsur1, $fangsur2, $fangsur3, $fangsur4, $flunas, $ftotalbayar, $frefer, $fnama, $tfHP, $fnamabank, $fnorekening, $ftempat, $ftgllahir, $falamat, $fkota, $fkodepos, $fprop, $femail, $fpassword, $fpaspor, $fnamarefer, $faktif, $ftglaktif, $fatasnama, $fkotabank, $ftgldaftar,  $ftglentry, $fcount, $fcabbank, $fcountrybank, $fswift, $flastuser, $flastupdate, $ftitle, $fsex, $fnation, $ffoto, $fdoc, $fnoktp, $fcountry, $fket, $fnpwp,$fpaket, $fpaketday,$fjenpay,$fprogram,$fprice,$fairporttax,$fassure,$fkakek,$fayah,$fibu,$fkec,$fdes,$fkurslunas,$ftgldepart,$farabassure,$fwarisbuss,$fpromo,$vTtd,$fktpdoc,$db)==1) {  
			...
			if ($vSelisih != 0) {
				   $vExisting = $oMember->isExistPayTab('sawal',$fidmember);
				   if ($vExisting == "") {
				   			$vDesc = "Setoran Awal";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$fstorawal,0,$fstorawal,'sawal') ;
				   } else {		        
				   			$vDesc = "Koreksi Setoran Awal";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$vSelisih,0,$fstorawal,'sawal') ;					   
				   }
			}
	  	 //   exit;
			$vSelisih = $fangsur1 - $vCurrAngs1;
			if ($vSelisih != 0) {
				   $vExisting = $oMember->isExistPayTab('angs1',$fidmember);
				   if ($vExisting == "") {
				   			$vDesc = "Angsuran 1";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$fangsur1,0,$fangsur1,'angs1') ;
				   } else {		        
				   			$vDesc = "Koreksi Angsuran 1";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$vSelisih,0,$fangsur1,'angs1') ;					   
				   }
			}
			...
			if ($vSelisih != 0) {
				   $vExisting = $oMember->isExistPayTab('angs2',$fidmember);
				   if ($vExisting == "") {
				   			$vDesc = "Angsuran 2";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$fangsur2,0,$fangsur2,'angs2') ;
				   } else {		        
				   			$vDesc = "Koreksi Angsuran 2";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$vSelisih,0,$fangsur2,'angs2') ;					   
				   }
			}
			...
			if ($vSelisih != 0) {
				   $vExisting = $oMember->isExistPayTab('angs3',$fidmember);
				   if ($vExisting == "") {
				   			$vDesc = "Angsuran 3";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$fangsur3,0,$fangsur3,'angs3') ;
				   } else {		        
				   			$vDesc = "Koreksi Angsuran 3";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$vSelisih,0,$fangsur3,'angs3') ;					   
				   }
			}

			$vSelisih = $fangsur4 - $vCurrAngs4;
			if ($vSelisih != 0) {
				   $vExisting = $oMember->isExistPayTab('angs4',$fidmember);
				   if ($vExisting == "") {
				   			$vDesc = "Angsuran 4";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$fangsur4,0,$fangsur4,'angs4') ;
				   } else {		        
				   			$vDesc = "Koreksi Angsuran 4";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$vSelisih,0,$fangsur4,'angs4') ;					   
				   }
			}
			...
			if ($vSelisih != 0) {
				   $vExisting = $oMember->isExistPayTab('lunas',$fidmember);
				   if ($vExisting == "") {
				   			$vDesc = "Pelunasan";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$flunas,0,$flunas,'lunas') ;
				   } else {		        
				   			$vDesc = "Koreksi Pelunasan";
				   			$oKomisi->insertPaymentTab($fidmember,$fidmember,date("Y-m-d H:i:s"),$vDesc,$vSelisih,0,$flunas,'lunas') ;					   
				   }
			}
	...
	if($db->query('COMMIT;')) {	  
	   $oSystem->jsAlert("Update sukses untuk ID Tabungan Jamaah $fidmember!");	
...
//$('.year').css("max-width", "90px");
 ...
 // $.validator.messages.required = '<span style="color:red;font-weight:normal">This field is required!</span>';
  ...
  $('#frmReg input, #frmReg textarea,  #frmReg select, #frmReg checkbox, #frmReg radio').not([type="submit"]).not($("#fidmember")).not($("#femail")).not($("#frefer")).not($("#fpropo")).not($("#fkota")).not($("#fpaspor")).not($("#tfBranchBank")).not($("#tfSernoUpName")).not($("#fttd")).not($("#fktpdoc")).addClass('required');  
...
function removeSpecChar(e) {
 ...
 <div class="right_col" role="main">
		...
		<div><label>
		<h3>Edit Tabungan Jamaah</h3></label></div> 
...
<div class="divtr col-lg-6" >
								...
								Nama Ibu*</label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-user"></i></span>
                                <input type="text" class="form-control" id="fibu" name="fibu" placeholder="(Tanpa tanda koma)*" onBlur="" onKeyUp="" value="<?=$dbL->f('fibu')?>">
                                </div>
</div> 
...
<div class="col-md-5 col-lg-6 divtr" >
 ...
 <option value="">--Pilih Jenis--</option>
							...
							<label for="exampleInputEmail1" >ID Tabungan Jamaah*  
...
<div class="col-md-5 col-lg-6 divtr" >
                                 ...
                                 <select class="form-control m-bot15" id="fpaket" name="fpaket" disabled>
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
								<? 
...
<select class="form-control m-bot15" id="fprogram" name="fprogram" onChange="changeProg(this);" disabled>
                            <option  value="" selected="selected" >--Pilih / Choose--</option>
                                <?
	...
	<div class="col-md-12 col-lg-12 divtr " <? if ($vProgramL !='4') echo 'style="display:none"';?> >
                            ...
                            <select class="form-control" id="fpromo" name="fpromo" disabled>
...
<div class="divtr col-lg-6">
                                  <label for="exampleInputEmail1" >Tanggal Berangkat*  </label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-calendar"></i></span>
                                         <input autocomplete="off"   id="ftgldepart" name="ftgldepart" class="form-control " placholder="YYYY-MM-DD"   type="text" ata-format="YYYY-MM-DD" data-template="YYYY MMM D" value="<?=$dbL->f('ftgldepart')?>"  disabled >
                                    </div>
</div>
...
<div style="" class=" divtr col-lg-6">
  ...
  <div style="" class=" divtr col-lg-12">
							...
							<label for="tfSernoSponName">Tanda Tangan </label>

										<div class="input-group">

                                      <span class="input-group-addon"> <i class="fa fa-money"></i></span>                                    

                                      <input type="file" class="form-control" id="fttd" name="fttd" placeholder="0" value=""  dir="rtl" <? if($vPriv!='administrator') echo 'readonly';?>>
  ...
  <div style="" class=" divtr col-lg-12">
							...
							<label for="tfSernoSponName">Tanda Tangan Tersimpan </label>

										<div class="input-group">

                                                                        

                                     <a href="<?=$dbL->f('fttd')?>" target="theblank"> <img src="<?=$dbL->f('fttd')?>"  width="150"  ></a>
  ...
  <div style="" class=" divtr col-lg-12">
							...
							<label for="tfSernoSponName">Scan atau Foto KTP </label>

										<div class="input-group">

                                      <span class="input-group-addon"> <i class="fa fa-money"></i></span>                                    

                                      <input type="file"  class="form-control" id="fktpdoc" name="fktpdoc" placeholder="Scan / foto KTP" value="0"  dir="rtl" readonly accept="image/png, image/gif, image/jpeg , image/jpg"  >
 ...
 <div style="" class=" divtr col-lg-12">
							...
							<label for="tfSernoSponName">KTP Tersimpan </label>

										<div class="input-group">

                                                                        

                                     <a href="<?=$dbL->f('fktpdoc')?>" target="theblank"> <img src="<?=$dbL->f('fktpdoc')?>"  width="150"  ></a>
...
<div class="row">
	...
	<? if (preg_match("/listregooo.php/",$vRefer) || preg_match("/aktiftab.php/",$vRefer)) {?>
