...
$(document).ready(function(){
	...
	$('#tblMain').DataTable({
        ...
        buttons: [{
             					extend: 'excelHtml5',
								filename: 'BonusRegistrasiBrgkt<?=$tfFDepart?>',
								title: 'Bonus Registrasi Tanggal Berangkat <?=$tfFDepart?>'
						}	, 
			 				{ 
								extend: 'print',
								title: 'Bonus Registrasi Tanggal Berangkat <?=$tfFDepart?>',
...
<div class="row">
		<div><label>
		<h3>Diskon Registrasi berdasarkan Tanggal Keberangkatan</h3></label></div>  		       
...
<tr style="font-weight:bold">
  ...
  <td  width="197" align="center" nowrap="nowrap">ID Korwil / Subkorwil</td>
  <td  width="170" align="center">Nama Korwil / Subkorwil</td>
  <td  width="151" align="center">Jml Jamaah yg Didaftarkan</td>
  <td  width="174" align="center">Diskon per Registrasi</td>
  ...
  <tbody>      
        ...
        <tr style="color:<?=$vColor?>"><td style="text-align: right;vertical-align:middle" valign="middle" ><?=$vCount?></td>
		  ...
		  <?
		      ...
		      $vSQLArea="select concat(a.fprop,a.fkabkota,a.fkec) as farea from tb_korwil_area a 
						...
						while($db1->next_record()) {
							...
							$dbin1->query($vSQLProg);
									while($dbin1->next_record()) {
									  ...
									  $vSQLReg = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUser) and flunas > 0 and fprogram='$vIDProg' and fpaket='$vIDPaket' $vAnd ";
						...
						while($db1->next_record()) {
							...
							$dbin1->query($vSQLProg);
										while($dbin1->next_record()) {
											$vIDProg = $dbin1->f('fidprogram');										// $vSQLUserSub = "select fidmember from m_admin where fkorwil='$vIDKorwil'";
											$vSQLReg = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUserSub) and flunas > 0 and fprogram='$vIDProg' and fpaket='$vIDPaket' $vAnd ";
						...
						while($db1->next_record()) {
							...
							$dbin1->query($vSQLProg);
											while($dbin1->next_record()) {
												$vIDProg = $dbin1->f('fidprogram');												// $vSQLUserSub = "select fidmember from m_admin where fkorwil='$vIDKorwil'";
											 $vSQLReg = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUserSpon) and flunas > 0 and fprogram='$vIDProg' and fpaket='$vIDPaket'  $vAnd ";
		 ...
		 $vTotal=0;
						...
						while($db1->next_record()) {
							...
							$dbin1->query($vSQLProg);
								while($dbin1->next_record()) {
										$vIDProg = $dbin1->f('fidprogram');		
										$vSQLUser = "select fidmember from m_admin where fkorwil='$vIDKorwil'";
									 $vSQLKTP = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUser)  and fprogram='$vIDProg' and fpaket='$vIDPaket'  and flunas >0 $vAnd ";
						...
						if($tfFLevel == 'KOR') {
							echo "<div  style='margin-top:5px'></div><br><br>";
							...
							$dbin1->query($vSQLProg);
								...
								$vSQLSubkor ="select fidkorwil from m_korwil where fidupline = '$vIDKorwil'";
								 ...
								 $vSQLUserSub = "select fidmember from m_admin where fkorwil in ($vSQLSubkor)";								
									...
									$vSQLKTP = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUserSub)  and fprogram='$vIDProg' and fpaket='$vIDPaket' and flunas >0 $vAnd ";
	...
	echo "<br><br><br>";
						...
						while($db1->next_record()) {
						 ...
						 $vSQLUserSpon ="select a.fidmember from m_admin a left join m_anggota b on a.fidjamaah=b.fidmember where concat(b.fprop,b.fkota,b.fkec)  in($vSQLArea)";
									...
									$vSQLKTP = "select count(fidmember) as fjml from m_anggota where fidregistrar in ($vSQLUserSpon)  and fprogram='$vIDProg' and fpaket='$vIDPaket'  and flunas >0 $vAnd ";
