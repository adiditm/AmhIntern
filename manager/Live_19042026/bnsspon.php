...
$vAnd = "";
	if ($tfFFrom !='' && $tfFTo !='' )	{
		$vAndNL .= " and faktif='1' and date(ftglaktif) between '$tfFFrom' and  '$tfFTo' "	;
		$vAnd .= " and a.faktif='1' and b.fkind like '%lunas%' and b.fcredit > 0  and date(b.ftanggal) between '$tfFFrom' and  '$tfFTo' "	;
	}

   $vSQL="select  distinct frefer from m_anggota where 1 and frefer is not null and frefer <> '-' ";
...
<div class="row">
		<div><label>
		<h3>Diskon Referensi Non SP</h3></label></div>  		       
...
<label>Syarat diskon program</label>
<?
...
<tr style="font-weight:bold">
  ...
  <td  width="132" align="center" nowrap="nowrap">ID Pebisnis</td>
  <td  width="177" align="center">Nama Pebisnis</td>
  <td  width="138" align="center">Jml Referensi Jamaah </td>
  <td  width="210" align="center">Diskon per Jamaah</td>
  ...
  <tbody>      
		...
		?>
        <tr id="tr<?=strtoupper($vRefer)?>" style="color:<?=$vColor?>"><td style="text-align: right;vertical-align:middle" valign="middle" ><?=$vCount?></td>
		  ...
		  <?
						...
						while($db1->next_record()) {
							$vIDPaket = $db1->f('fpackid');
								...
								if ($vSyaratLns !=0) {
									 $vSQLKTP = "select count(*) as fjml from(select a.fidmember, min(b.ftanggal) from m_anggota a left join tb_payhist b on a.fidmember=b.fidmember where b.ftanggal is not null and a.frefer='$vRefer'  and a.fprogram='$vIDProg' and  a.fpaket='$vIDPaket'  and a.flunas > 0  $vAnd group by fidmember) as a ";
								} else {
									  $vSQLKTP = "select count(fidmember) as fjml from m_anggota where frefer='$vRefer'  and fprogram='$vIDProg' and  fpaket='$vIDPaket'  and fstorawal >= $vSyaratSA $vAndNL ";									
								}
			 ...
			 if ($vTotal <=0) {
			  ...
			  if ($vPost =='1' && $vTotal >0) {
				 ...
				 $dbin->query($vSQL);
 					...
 					$vDesc="Bonus Sponsor Periode $vFromKet s/d $vToKet";		
				  $vLastBal=$oKomisi->getLastBalanceBis($vIDBisnis);
				 
				  $vBal=$vLastBal + $vNetto;
				  $oKomisi->insertMutasi($vRefer,$vRefer,date("Y-m-d H:i:s"),$vDesc,$vNetto,0,$vBal,'spon',$pIDJual) ;
				  $oMember->updateBalBis($vRefer,$vBal);						 
			  }			  
...
<script language="javascript">
   ...
   $(document).ready(function(){
		...
		<? if (count($vArrZero) == $vNumRows) {?>
			...
			alert('Tidak ada yang mendapatkan diskon referensi pada periode ini!');
