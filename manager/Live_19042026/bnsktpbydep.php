...
$(document).ready(function(){
	...
	$('#tblMain').DataTable({
        ...
        buttons: [{
             					extend: 'excelHtml5',
								filename: 'BonusKTPDepBrgkt<?=$tfFDepart?>',
								title: 'Bonus KTP Tanggal Berangkat <?=$tfFDepart?>'
						}	, 
			 				{ 
								extend: 'print',
								title: 'Bonus KTP Tanggal Berangkat <?=$tfFDepart?>',
...
<div class="row">
		<div><label>
		<h3>Diskon KTP berdasarkan Tanggal Keberangkatan</h3></label></div>  		       
		      ...
		      $vSQLArea="select concat(a.fprop,a.fkabkota,a.fkec) as farea from tb_korwil_area a 
						...
						while($db1->next_record()) {
							...
							$dbin1->query($vSQLProg);
								while($dbin1->next_record()) {
									$vIDProg = $dbin1->f('fidprogram');
									$vSQLKTP = "select count(fidmember) as fjml from m_anggota where concat(fprop,fkota,fkec) in ($vSQLArea)  and fprogram='$vIDProg' and fpaket='$vIDPaket' and flunas > 0 $vAnd ";
