...
<div class="row">
		<div><label>
		<h3>Diskon Referensi berdasarkan Tanggal Berangkat  SP</h3></label></div>  		       
...
<tr style="font-weight:bold">
  ...
  <td  width="174" align="center">Jml Referensi Jamaah </td>
  <td  width="215" align="center">Setting Diskon per Jamaah</td>
  ...
  <tbody>      
        ...
        <tr style="color:<?=$vColor?>"><td style="text-align: right;vertical-align:middle" valign="middle" ><?=$vCount?></td>
		  ...
		  <?
						...
						while($db1->next_record()) {
							...
							if ($vSyaratLns !=0) {											
								...
								$vSQLTarget = "select * from tb_promo where fdepart='$tfFDepart'";
								$dbin->query($vSQLTarget);
								$dbin->next_record();
								$vTargetNum = $dbin->f('fminspon');
								$vPrice = $dbin->f('fprice');
						...
						$vStatusTarget = "";
						echo "<br>Total SP: ".number_format($vJmlSP,0,",",".");
						if ($vJmlSP==$vTargetNum) {
							$vStatusTarget="Memenuhi Target";
							$vStat="OnTarget";
						}
						if ($vJmlSP<$vTargetNum) {
						    $vStatusTarget="Kurang dari Target";		
							$vStat="UnderTarget";
						}
						if ($vJmlSP>$vTargetNum){
						    $vStatusTarget="Melebihi Target $vTargetNum Jamaah";		
							$vStat="OverTarget";
						}
							
						echo "<br>($vStatusTarget)";	
		 ...
		 <? 
								...
								if ($vStat=='UnderTarget') {
										$vBonus =  $oRules->getBnsSetting('SPON',$vIDProg, $tfFLevel,$vIDPaket) ;
										$vTotal += $vJmlSP * $vBonus;
										$vRinci= "<br><font color='#00f'>($vJmlSP x ".number_format($vBonus,0,",",".").")</font>";
								} else if ($vStat=='OnTarget')	 {
									if ($vTargetNum >0)
										$vBonus = $vPrice / $vTargetNum;
									else $vBonus=0;	
									$vTotal += $vJmlSP * $vBonus;
									$vRinci= "<br><font color='#00f'>($vJmlSP x ".number_format($vBonus,0,",",".").")</font>";
								} else if ($vStat=='OverTarget') {
									$vKelipatan = floor($vJmlSP / $vTargetNum);
									$vSisa = $vJmlSP % $vTargetNum;
									$vSelisih = $vSisa;
									$vBonus1 = $oRules->getBnsSetting('SPON',$vIDProg, $tfFLevel,$vIDPaket) ;
								    $vTotal1 = $vSelisih *  $vBonus1;
									$vBakalBonus = $vPrice / $vTargetNum;
									$vBonus2 = $vBakalBonus* $vKelipatan ;
									$vTotal2 =  $vTargetNum * $vBonus2;
									//echo "<br>$vBonus1<br>$vBonus2";
									$vBonus = $vBonus1 + $vBonus2;
									$vTotal += $vTotal1 + $vTotal2;
									$vRinci= "<br><font color='#00f'> ($vKelipatan x $vTargetNum x  ".number_format($vBakalBonus,0,",",".").") + ($vSelisih x  ".number_format($vBonus1,0,",",".").")</font>";
