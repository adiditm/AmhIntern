...
$vAnd = "";
	if ($tfFFrom !='' && $tfFTo !='' ){
		$vAndNL .= " and faktif='1' and ftglaktif between '$tfFFrom' and  '$tfFTo' "	;
		$vAnd .= " and a.faktif='1' and b.fkind like '%lunas' and date(b.ftanggal) between '$tfFFrom' and  '$tfFTo' "	;
	}
...
<div class="row">
		<div><label>
		<h3>Diskon Referensi SP</h3></label></div>  		       
						...
						while($db1->next_record()) {
							$vIDPaket = $db1->f('fpackid');
								...
								$dbin1->query($vSQLProg);
									...
									$vSyaratLns = $dbin1->f('fsyaratlns');
								if ($vSyaratLns !=0) {	
									$vSQLKTP = "select count(a.fidmember) as fjml from m_anggota a left join tb_payhist b on a.fidmember=b.fidmember where a.frefer='$vRefer'  and a.fprogram='$vIDProg' and  a.fpaket='$vIDPaket'  and a.flunas > 0 $vAnd ";
								} else {
		 ...
		 <? 
			 ...
			 if ($vTotal <=0) {
			  ...
			  if ($vPost =='1' && $vTotal >0) {
				 ...
				 $vSQL .= "VALUES ( '$tfFFrom','$tfFTo','$vRefer', 'system',$vTotal , 'SPON', '0', 'Bonus Sponsor (SP) Periode $vFromKet s/d $vToKet', '$vRef',now(),'',$vNetto );";  
				 
				 $dbin->query($vSQL);

					$vDesc="Bonus Sponsor (SP) Periode $vFromKet s/d $vToKet";		
				  $vLastBal=$oKomisi->getLastBalanceBis($vIDBisnis);
				 
				  $vBal=$vLastBal + $vNetto;
				  $oKomisi->insertMutasi($vIDBisnis,$vIDBisnis,date("Y-m-d H:i:s"),$vDesc,$vNetto,0,$vBal,'spon',$pIDJual) ;
...
<script language="javascript">
   ...
   $(document).ready(function(){
		...
		<? if (count($vArrZero) == $vNumRows) {?>
			...
			alert('Tidak ada yang mendapatkan diskon referensi SP pada periode ini!');
