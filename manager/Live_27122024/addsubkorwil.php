...
//print_r($_POST);
 // $_SESSION['Ref']='';
	...
	$dbL = $db1;
     ...
     $vSQL="select a.* from m_korwil a  where a.fidkorwil='$vUserActive'";
 
 	$dbL->query($vSQL);
 	$dbL->next_record();

   	$vIdPilgrimsL = $dbL->f('fidkorwil');
...
function generateRandomString($length = 6) {
   ...
   if ($_POST['hPost'] != '1') {
	 ...
	 $oSystem->smtpmailer('japri_s@yahoo.com',$vMailFrom,'AMH',"Registrasi AMH Internal",'','',false);
	  ...
	  $vhSubTot=0;	  	 
		 ...
		 $flastuser = $_SESSION['LoginUser'];
			 ...
			 if ($oMember->regKorwil($fidkorwil, $flevel, $fnama, $fnohp, $falamat, $fprop,$fkota, $fkec, $femail, $fnoktp, $fket,$fidupline,$fidbisnis,$db)==1) {  
	...
	if($db->query('COMMIT;')) {	  
	   $oSystem->jsAlert("Penambahan data Korwil sukses!");	
...
//	if(typeof $('#hTotJum').val() !== "undefined") {
 ...
 /*var vPaket=document.getElementById('rbPaket').value;
		    ...
		    if (confirm('Anda yakin melakukan penambahan data korwil?')==true) {
...
<? } ?>
 ...
 <div class="right_col" role="main">
		...
		<div><label>
		<h3>Menambah Sub Korwil</h3></label></div> 
                     ...
                     <div class="panel-body" style="color:black">
							...
							<label for="exampleInputEmail1" >ID
							 ...
							 <div align="left" style="display:inline" id="statKit"></div></label>
                                    ...
                                    <input onKeyUp="this.value=this.value.toUpperCase()"  type="text" class="form-control" id="fidkorwil" name="fidkorwil" value="<?=$vIdPilgrimsL?>"    >
                              </div>                            
                            </div>
...
<div class="col-md-5 col-lg-6 divtr" >
									  ...
									  <span style="font-weight:bold">&nbsp;Tingkatan*</span></label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-gift"></i></span>
                                 <select class="form-control m-bot15" id="flevel" name="flevel" onChange="if(this.value=='KOR') document.getElementById('fidupline').disabled=true; else document.getElementById('fidupline').disabled=false; ">
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
								<? 
								    $vSQL="select * from m_level_korwil where flevelid='SUBKOR' order by fidsys";
								    $db->query($vSQL);
								    while ($db->next_record()) {
								?>                               
								 <option value="<?=$db->f('flevelid')?>" <? if($db->f('flevelid')==$vLevelL) echo 'selected'; ?>  ><?=$db->f('flevelname')?></option>
								 <? } ?>
                            </select>
                           </div>
                          </div>
                          ...
                          <div class="divtr col-lg-6" >
                                <label   for="tfNama">
								Nama Sub Korwil*</label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-user"></i></span>
                                <input type="text" class="form-control" id="fnama" name="fnama" placeholder="(Tanpa tanda koma)*" onBlur="this.value=this.value.replace(/,/g,'');$('#tfAtasNama').val(this.value);" onKeyUp="this.value=this.value.replace(/,/g,'');$('#tfAtasNama').val(this.value);" value="<?=$vNamaL?>">
                                </div>
</div>    
        
        <div class="col-md-5 col-lg-6 divtr" >
                                      <label for="exampleInputEmail1">
									  <span style="font-weight:bold">&nbsp;Atasan*</span></label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-gift"></i></span>
                                 <select class="form-control m-bot15" id="fidupline" name="fidupline">
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
								<? 
								    $vSQL="select * from m_korwil where flevel='KOR' order by fidkorwil";
								    $db->query($vSQL);
								    while ($db->next_record()) {
								?>                               
								 <option value="<?=$db->f('fidkorwil')?>" <? if($db->f('fidkorwil')==$vUplineL) echo 'selected'; ?>  ><?=$db->f('fnama')?></option>
								 <? } ?>
                            </select>
                           </div>
                          </div>
 ...
 <div class="divtr col-lg-6 ">
                                    ...
                                    <div class="input-group">
                                      ...
                                      <span class="input-group-addon"> <i class="fa fa-id-card"></i></span>
                                    	...
                                    	<input type="number" maxlength="16" size="16" class="form-control" id="fnoktp" name="fnoktp" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" placeholder="No. KTP.*" value="<?=$vNoKTP?>">
...
<div class="col-md-5 col-lg-6 divtr" >
                                      <label for="exampleInputEmail1">
									  <span style="font-weight:bold">&nbsp;ID Pebisnis*</span></label>
                                    <div class="input-group">
                                      <span class="input-group-addon"> <i class="fa fa-gift"></i></span>
                                 <select class="form-control m-bot15" id="fidbisnis" name="fidbisnis">
                                <option  value="" selected="selected" >--Pilih / Choose--</option>
								<? 
								    $vSQL="select kode,nama from tmmember  order by kode";
								    $oDBAMHT->query($vSQL);
								    while ($oDBAMHT->next_record()) {
								?>                               
								 <option value="<?=$oDBAMHT->f('kode')?>" <? if($oDBAMHT->f('kode')==$vPebisnisL) echo 'selected'; ?>  ><?=$oDBAMHT->f('kode')?> / <?=$oDBAMHT->f('nama')?></option>
								 <? } ?>
                            </select>
                           </div>
                          </div>                             
                   ...
                   <div class="form-group" >     
                        ...
                        <div class="divtr col-lg-12">
                            ...
                            <label for="exampleInputEmail1">Alamat Korwil*</label>
...
<div class="row">
                     ...
                     <? } ?>
                                        ...
                                        <button type="button" class="btn btn-default" onClick="document.location.href='../masterdata/aktifsubkorwil.php?op=&current=mdm_master_data&menu=mdm_master_data_editkor';" >Back &laquo;</button> 
