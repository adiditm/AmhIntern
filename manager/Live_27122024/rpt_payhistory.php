<? 
   ...
   if ($vOP=='getbiz') {
	 ...
	 if (strlen($vQuery) >=2) {
		 $vSQL="select fidmember as id, fnama as nama  from m_anggota where fidmember like '%$vQuery%' or fnama like '%$vQuery%'";
		 $db->query($vSQL);
		 $vArrData = array();
		 $vCount=0;
		 while($db->next_record()) {
			$vArrData[$vCount]['id']=$db->f('id');
			$vArrData[$vCount]['text'] = $db->f('id')." / ".$db->f('nama'); 
			$vCount++;
		 }
		 
		 echo json_encode($vArrData);
	 }
 ...
 if ($vSearch!="")	{	
  ...
  $vSQL="select count(*) as fjumrec from tb_payhist where fkind in('sawal','angs1','angs2','angs3','angs4','flunas','sawal','lunas','fangsur1','fangsur2','fangsur3','fangsur4','handle','assure','arabassure','voucher') and fidmember like '%$vUserChoosed%' $vCrit";
 $db->query($vSQL);
 $db->next_record();
...
$('[data-toggle="tooltip"]').tooltip({tooltipClass:"ttclass"});  
 ...
 $('#lmBiz').select2({
        ...
        placeholder: '--Pilih Jamaah (pencarian min. 2 huruf)--',

        ajax: {

          url: 'rpt_payhistory.php?op=getbiz',
          ...
          var data = $.map(response, function (obj) {
                ...
                obj.id = obj.id || obj.fidrent; // replace pk with your identifier
...
<div class="right_col" role="main">
		...
		<div><label>
		<h3>History Pembayaran </h3></label></div>  		       
...
<div class="row">
            ...
            <div align="left" class="col-sm-4"><strong>Date </strong></div>
              ...
              <input autocomplete="off" class="dpick form-control form-inline"  name="dc"  id="dc"  value="<?=$vStart?>"   />
              ...
              <input autocomplete="off"  class="dpick form-control form-inline" name="dc1" id="dc1" value="<?=$vEnd?>" /></div>
...
<br>
         ...
         <div class="row">
            <div align="left" class="col-sm-4"><strong>Jamaah </strong></div>
            <div align="left" class="col-sm-6 col-md-6 col-xs-6">
        <select name="lmBiz" id="lmBiz" class="form-control" onfocus="$('.select2-selection__rendered').show();">
                <option  value="">--Pebisnis / Korwil--</option>
                <? if ($_POST['lmBiz'] !='') { ?>
                	<option selected="selected" value="<?=$_POST['lmBiz']?>"><?=$_POST['lmBiz']?></option>
...
<br>
        ...
        <tr style="font-weight:bold">
          ...
          <td width="7%" align="center" class="auto-style1">Date</td>
          <td width="39%" align="center" class="auto-style1">Jamaah</td>



          <td width="24%" align="center" class="auto-style1">Description</td>
          ...
          <td width="8%" align="center" class="hide">Debit</td>



          <td width="11%" align="center" class="hide">Balance</td>
        </tr>
        ...
        <?
			    ...
			    $vSQL="select * from tb_payhist where   fkind in('fstorawal','fangsur1','fangsur2','fangsur3','fangsur4','flunas','sawal','lunas','handle','assure','farabassure','voucher') and fidmember like '%$vUserChoosed%'   ";
				...
				while ($db->next_record()) {
				   ...
				   $vNama=$oMember->getMemberName($vJamaah);
        ...
        <tr class="tbrekapfont">
          ...
          <td  align="left"><?=$vJamaah?> <? if ($vNama !='' && $vNama != -1) echo " / $vNama";?></td>
...
<?=number_format($vBal,0,",",".");?>
		...
		<? if ($db->num_rows() <=0){ ?>
		      <tr class="tbrekapfont">
          <td colspan="7"  align="center">Tidak ada history, atau pembayaran Anda belum disetujui!</td>
          </tr>
		  <? } ?>
      </table>
  ...
  <button style="margin-left:2em" class="btn btn-info btn-sm" onClick="document.location.href='../manager/getexcel.php?arr=ewallet&file=ewallet_report_<?=$vUserChoosed?>'"><i class="fa fa-file-text-o"></i> Export Excel</button>  
   <button id="btnBack" type="button" class="btn btn-default"  onClick="document.location.href='<?=$_SERVER['HTTP_REFERER']?>'">Back</button>
