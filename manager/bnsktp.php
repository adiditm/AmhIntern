...
$bulan = array(
  ...
  if ($vRefURL=="" && $_SESSION['LoginUser'] !='ADMIN_2')
...
$vAnd = "";
	if ($tfFFrom !='' && $tfFTo !='' )	
		$vAnd .= " and a.faktif='1' and b.fkind like '%lunas' and b.fcredit > 0 and date(b.ftanggal) between '$tfFFrom' and  '$tfFTo' "	;
...
function doPost(pParam){
   ...
   if (parseFloat(pParam) >0) {
	   if(confirm('Anda yakin melakukan posting bonus KTP periode '+vFrom+' s/d '+vTo+'?')) {
...
$(document).ready(function(){
				//	$('#tfFFrom').val('<?=date('Y-m-01')?>');	
			//		$('#tfFTo').val('<?=date("Y-m-t", strtotime(date('Y-m-d')))?>');	
...
<div class="col-lg-12"  >
      ...
      <form  method="post" id="frmRep" name="frmRep" enctype="multipart/form-data" style="color:black" >
        <div class="row"> 
               <div class="col-lg-2">
               <b> Mulai</b>  </div>
              <div class="col-lg-2">
 ...
 <div class="row"> 
        ...
        <?
            if ($vPost=='1') {
				$vSQLCheck = "select * from tb_komisi where '$tfFFrom' between fdfrom and fdto and fkind='KTP' and flevel='$tfFLevel' ";
				$dbin1->query($vSQLCheck);
				$dbin1->next_record();
				$vRowFrom = $dbin1->num_rows();
	
			 	$vSQLCheck = "select * from tb_komisi where '$tfFTo' between fdfrom and fdto and fkind='KTP' and flevel='$tfFLevel' ";
				$dbin1->query($vSQLCheck);
				$dbin1->next_record();
...
<tr style="font-weight:bold">
  ...
  <td  width="169" align="center">Jml Jamaah di Wilayah</td>
  <td  width="167" align="center">Diskon per KTP</td>
  ...
  <tbody>      
		...
		?>
        <tr id="tr<?=strtoupper($vIDKorwil)?>" style="color:<?=$vColor?>"><td style="text-align: right;vertical-align:middle" valign="middle" ><?=$vCount?></td>
						...
						while($db1->next_record()) {
							...
							while($dbin1->next_record()) {
								$vIDProg = $dbin1->f('fidprogram');
									 ...
									 //$vSQLKTP = "select count(a.fidmember) as fjml from m_anggota a left join tb_payment b on a.fidmember=b.fidmember where concat(a.fprop,a.fkota,a.fkec) in ($vSQLArea)  and a.fprogram='$vIDProg' and a.fpaket='$vIDPaket' and flunas > 0 $vAnd ";
									 $vSQLKTP = "select count(a.fidmember) as fjml from m_anggota a left join tb_payhist b on a.fidmember=b.fidmember where concat(a.fprop,a.fkota,a.fkec) in ($vSQLArea)  and a.fprogram='$vIDProg' and a.fpaket='$vIDPaket' and flunas > 0 $vAnd ";
								//	if ($_SESSION['LoginUser']=='ADMIN_2') echo $vSQLKTP;
						...
						while($db1->next_record()) {
							$vIDPaket = $db1->f('fpackid');
								...
								while($dbin1->next_record()) {
										...
										$vSQLKTP = "select count(a.fidmember) as fjml from m_anggota a left join tb_payhist b on a.fidmember=b.fidmember where concat(a.fprop,a.fkota,a.fkec) in ($vSQLArea)  and a.fprogram='$vIDProg' and  a.fpaket='$vIDPaket' and a.flunas >0 $vAnd ";
									...
									//echo $vSQLKTP ;	
										...
										$vBonus =  $oRules->getBnsSetting('KTP',$vIDProg, $tfFLevel,$vIDPaket) ;
										$vTotal += $vJml * $vBonus;
		  ...
		  <?
			  ...
			  $vRef="POSTKTP-".date("Ymd-His");
			  if ($vTotal <=0) {
				$vArrZero[]=strtoupper($vIDKorwil); 
			 }
			  if ($vPost =='1' && $vTotal >0) {
				 ...
				 $vSQL="INSERT INTO tb_komisi(fdfrom, fdto, fidmember, fidfunder, famount, fkind, ffeestatus, fdesc, fref, ftanggal, flevel, fidkorwil, fnetto) ";
				  $vSQL .= "VALUES ( '$tfFFrom','$tfFTo','$vIDBisnis', 'system',$vTotal , 'KTP', '0', 'Bonus KTP Periode $vFromKet s/d $vToKet', '$vRef',now(),'$tfFLevel','$vIDKorwil', $vNetto );";  
				 
				 $dbin->query($vSQL);
				 
				 
				  $vDesc="Bonus KTP Periode $vFromKet s/d $vToKet ";		
				  $vLastBal=$oKomisi->getLastBalanceBis($vIDBisnis);
				 
				  $vBal=$vLastBal + $vNetto;
				  $oKomisi->insertMutasi($vIDBisnis,$vIDBisnis,date("Y-m-d H:i:s"),$vDesc,$vNetto,0,$vBal,'ktp',$pIDJual) ;
    ...
    <?		
		 ...
		 if ($vPost=='1') {
			    if($dbin1->query("COMMIT;")) {
				   $oSystem->jsAlert("Posting bonus KTP berhasil!");	
				   $oSystem->jsLocation($_SERVER['HTTP_REFERER']);
				}
...
<script language="javascript">
   ...
   $(document).ready(function(){
		<? foreach($vArrZero as $key=>$val) {?>
		
		$('#tr<?=$val?>').hide();
		
		<? } ?>
		
		<? if (count($vArrZero) == $vNumRows) {?>
		
			alert('Tidak ada yang mendapatkan diskon registrasi pada periode ini!');
