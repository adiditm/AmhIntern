  ...
  function sendMail($pFrom,$pToAddr,$pName,$pBcc,$pSubject,$pMessage,$pSMTP,$pFromName) {
	...
	function getOutletName($pID) {
		...
		function authAdminPin($pID,$pPin) {		
		    ...
		    $vsql="SELECT fidmember from m_admin where fidmember='$pID' and fpin='$pPin' and faktif='1' ";	
   ...
   //Set Password Admin
     ...
     function setAdminPassConfirm($pID,$pNewPass,$pConfirm){
		...
		function authAdmin($pID,$pPass) {		
		    ...
		    $vsql="SELECT fidmember from m_admin where fidmember='$pID' and fpassword='$pPass' and faktif='1' ";	
		...
		function authAdminNP($pID) {		
		    ...
		    $vsql="SELECT fidmember from m_admin where fidmember='$pID' and faktif='1'  ";	
...
//Set Counter
		...
		function getUserAdmin() {
		    ...
		    $vsql="select fidmember, fnama from m_admin where fpriv='administrator' or fpriv='seller'";	
		...
		function getUserMenu($pUser,$pPriv) {
			...
			if ($pPriv==false)
		         ...
		         $vsql="select menu_id,menu_title,fparent,fismenu,fhassub from m_menu where is_active=1 and menu_id not in (select menu_id from tb_menupriv where user_id='$pUser') order by menu_title";	

			else

			    $vsql="select b.menu_id,a.menu_title,fparent,fismenu,fhassub from m_menu a,tb_menupriv b where a.is_active=1 and a.menu_id=b.menu_id and b.user_id='$pUser' order by menu_title ";
			...
			while ($oDB->next_record()) {
				...
				$vres['fparent'][] = $oDB->f("fparent");
				$vres['fismenu'][] = $oDB->f("fismenu");
				$vres['fhassub'][] = $oDB->f("fhassub");
	...
	function getGoto($pURL) {
		...
		function smtpmailerHosting($to, $from, $from_name, $subject, $body,$bcc='',$bcc2='',$html=true,$debug=0) {
			...
			$mail->SMTPDebug = $debug;  // untuk memunculkan pesan error /debug di layar
			...
			//$mail->Username = $from; //username email
			$mail->Username = $from; //username email
    ...
    //Sync Korwil
		function syncKorwil() {
			...
			while ($oDBAMHT->next_record()) {
				...
				$vAlamat = str_replace("'","''",$oDBAMHT->f("jalamat"));
				...
				$vSQLIn .=" VALUES  ('$vKode', '$vKode', '$vNama', '$vHP', '$vBank', '$vNorek', '$vAtasNama', '$vAlamat', 'ID','','','', '$vEmail', '1', '$vTanggal', '', '');";
				 ...
				 $vMsgAll .=$vMsg;
					...
					if ($oDB->frefer != $vKode || $oDB->fnama != $vNama || $oDB->fnohp != $vHP || $oDB->fnamabank != $vBank || $oDB->fnorekening != $vNorek || $oDB->fatasnama != $vAtasNama || $oDB->falamat != $vAlamat || $oDB->femail != $vEmail) {
					 $vSQLIn = "update m_pebisnis set frefer='$vKode',  fnama= '$vNama', fnohp='$vHP', fnamabank='$vBank', fnorekening='$vNorek', fatasnama='$vAtasNama', falamat='$vAlamat', fcountry='ID',  femail='$vEmail',   fket='$vUpdatedDate' where fidmember='$vKode'";
					  $oDB->query($vSQLIn);
